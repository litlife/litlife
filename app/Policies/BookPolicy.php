...
class BookPolicy extends Policy
{
	...
	public function update(User $auth_user, Book &$book)
	{
		...
		$this->deny(__('book.you_do_not_have_the_right_to_edit_the_description_of_this_book'));
	}
	...
	{
		if ($book->trashed())
			// книга уже удалена, поэтому запрещаем
			return false;
	...
	{
		if (!$book->trashed())
			// книга не была удалена, поэтому запрещаем
			return false;
	...
	*/
	...
	public function view_section_list(?User $auth_user, Book $book)
	{
		...
		if ($book->isPrivate()) {
			...
			$this->deny(__('book.access_to_the_book_is_limited'));
		}
	...
	{
		if (@$auth_user->getPermission('CheckBooks'))
			// пользователю можно проверять книги
			return true;
	...
	public function read(?User $auth_user, Book $book)
	{
		...
		if ($book->isForSale()) {
			...
			if (empty($auth_user))
				$this->deny(__('book.paid_part_of_book'));
	...
	public function download(?User $auth_user, Book $book)
	{
		...
		if ($book->isForSale()) {
			if (empty($auth_user))
				$this->deny(__('book.you_need_to_purchase_a_book_to_download'));
	...
	public function change_sell_settings(User $auth_user, Book $book)
	{
		...
		if ($book->isRejected())
			$this->deny(__('book.removed_from_sale'));
	...
	public function sell(User $auth_user, Book $book)
	{
		...
		if ($book->getAuthorsWithType('writers')->count() > 1)
			$this->deny(__('book.we_dont_have_the_opportunity_to_sell_books_with_more_than_one_writer'));
		...
		if ($manager = $book->getManagerAssociatedWithUser($auth_user)) {
			if ($manager->can_sale) {
				...
				$this->deny(__('book.to_sell_books_you_need_to_request'));
		} else
			return false;
	}
	...
	public function buy(User $auth_user, Book $book)
	{
		...
		if (empty($seller_manager))
			$this->deny(__('book.sales_of_this_book_are_temporarily_disabled'));

		if (optional($book->getManagerAssociatedWithUser($auth_user))->character == 'author')
			$this->deny(__('book.you_can_not_buy_a_book_that_you_sell'));
		else {
			if ($book->purchases->where('buyer_user_id', $auth_user->id)->first()) {
				$this->deny(__('book.you_already_buy_this_book'));
			} else {
				return true;
			}
		}
