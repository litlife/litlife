...
class BookKeywordPolicy extends Policy
{
    ...
    {
        if (!$keyword->isSentForReview()) // ключевое слово не на проверке
        {
            return false;
        }
    ...
    {
        if ($keyword->trashed()) // ключевое слово уже удалено
        {
            return false;
        }
        ...
        if ($keyword->isSentForReview() or $keyword->isPrivate()) {
            if ($keyword->isUserCreator($auth_user)) // пользователь может удалять свои еще не проверенные ключевые слова
            {
                return true;
            }
        } else {
