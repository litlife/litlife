...
class UserPolicy extends Policy
{
    ...
    public function update(User $auth_user, User $user)
    {
        if ($auth_user->id == $user->id) {
            if ($auth_user->getPermission('edit_profile')) // это профиль пользователя и он может его редактировать
            {
                return true;
            }
        } else {
            if ($auth_user->getPermission('edit_other_profile')) // это чужой профиль пользователя и его можно редактировать пользователю
            {
                return true;
            }
        }
    ...
    public function writeOnWall(User $auth_user, User $user)
    {
        ...
        if ($auth_user->id == $user->id) // можно писать на своей стене
        {
            return true;
        }
    ...
    public function write_private_messages(User $auth_user, User $user)
    {
        ...
        if ($auth_user->id == $user->id) // нельзя писать себе личные сообщения
        {
            return false;
        }
    ...
    public function block(User $auth_user, User $user)
    {
        ...
        if ($auth_user->hasAddedToBlacklist($user)) // пользователь уже в черном списке
        {
            return false;
        }
    ...
    public function unblock(User $auth_user, User $user)
    {
        ...
        if (!$auth_user->hasAddedToBlacklist($user)) // пользователь не найден в черном списке
        {
            return false;
        }
    ...
    {
        if ($user->on_moderate) // пользователь уже на модерации
        {
            return false;
        }
    ...
    {
        if (!$user->on_moderate) // пользователь нет на модерации
        {
            return false;
        }
...
*/
    ...
    public function takeSurvey(User $auth_user)
    {
        ...
        if ($auth_user->surveys()->first()) {
            return $this->deny(__('survey.you_have_already_passed_the_survey'));
        } else {
            return true;
        }
    }
