...
class CommentPolicy extends Policy
{
    ...
    public function reply(User $auth_user, Comment $comment)
    {
        ...
        if ($comment->create_user->hasAddedToBlacklist($auth_user)) {
            return $this->deny(__('comment.you_cant_reply_to_a_comment_if_the_user_is_blacklisted'));
        }

        if ($comment->isUserCreator($auth_user)) {
            return $this->deny(__('comment.you_cant_reply_to_your_comments'));
        } else {
            return true;
        }
    }
    ...
    public function vote(User $auth_user, Comment $comment)
    {
        ...
        /*
                if ($comment->isUserCreator($auth_user))
                    return $this->deny(__('comment.you_cant_vote_for_your_comments'));
        */
        if ($comment->create_user->hasAddedToBlacklist($auth_user)) {
            return $this->deny(__('comment.you_cant_vote_for_a_comment_if_the_user_is_blacklisted'));
        }
