...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/" mode="description">
        ...
        <fieldset style="border:none">
            <button id="tiShowBtn" onclick="ShowElementsMenu(this)" unselectable="on"
        ...
        <fieldset id="binobj" unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_BINARY_OBJ"/>
            </legend>
            <br/>
        </fieldset>
        <br/>
        <fieldset unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_STYLES_TABLE"/>
            </legend>
            <textarea id="stylesheetId" rows="10">
                <xsl:value-of select="/f:FictionBook/f:stylesheet"/>
            </textarea>
        </fieldset>
    </xsl:template>
    ...
    <xsl:template match="*">
        <span class="unknown_element" source_class="{local-name()}">
            <xsl:for-each select="@*">
                <xsl:attribute name="unknown_attribute_{name()}">
                    <xsl:value-of select="."/>
                </xsl:attribute>
            </xsl:for-each>
            <xsl:apply-templates/>
    ...
    <xsl:template name="ti">
        ...
        <fieldset unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_BOOK"/>
            </legend>
            <fieldset id="tiGenre" unselectable="on" class="kid">
                <legend unselectable="on">
                    <xsl:value-of select="$IDS_GENRES"/>
                </legend>
                    ...
                    <div unselectable="on">
                        ...
                        <xsl:if test="not(@match)">
                            <span id="ti_genre_match" unselectable="on">
                                <button onclick="ShowElement(this.parentNode.id, false)" unselectable="on"
                                        style="font-family : Tahoma;">-
                                </button>
                                <label unselectable="on">
                                    <xsl:value-of select="$IDS_MATCH"/>
                                </label>
                                <input type="text" maxlength="3" id="match" value="{@match}" class="short"/>
                            </span>
                        </xsl:if>
                        <xsl:if test="@match">
                            <label unselectable="on">
            ...
            <fieldset id="tiAuthor" unselectable="on" class="kid">
                <legend unselectable="on">
                    <xsl:value-of select="$IDS_AUTHORS"/>
                </legend>
            ...
            <br/>
            <label unselectable="on" class="hl">
                <xsl:value-of select="$IDS_TITLE"/>
            </label>
            <input type="text" maxlength="256" id="tiTitle" class="wide" value="{$cur/f:book-title}"/>
            <br/>
            <xsl:if test="not($cur/f:keywords)">
                <span id="ti_kw" unselectable="on">
                    ...
                    <label unselectable="on" desc_extended="true">
                        <xsl:value-of select="$IDS_KEYWORDS"/>
                    </label>
                    <input type="text" maxlength="256" id="tiKwd" class="wide" value="{$cur/f:keywords}"/>
                    <br/>
                </span>
            </xsl:if>
            ...
            <label unselectable="on">
                <xsl:value-of select="$IDS_DATE_TEXT"/>
            </label>
            <input type="text" maxlength="256" id="tiDate" value="{$cur/f:date}"/>
            <label unselectable="on" style="width:6.4em">
                <xsl:value-of select="$IDS_DATE_VALUE"/>
            </label>
            <input type="text" maxlength="256" id="tiDateVal" value="{$cur/f:date/@value}"/>
            <br/>
            <fieldset id="tiCover" unselectable="on" class="kid">
                <legend class="cover" unselectable="on">
                    <xsl:value-of select="$IDS_COVERPAGE"/>
                </legend>
                <xsl:for-each select="$cur/f:coverpage/f:image">
                    <div unselectable="on">
                        <button id="del" onclick="Remove(this.parentNode)" unselectable="on">&#x72;</button>
                        <button onclick="Clone(this.parentNode)" unselectable="on">&#x32;</button>
                        <button id="show" onmouseover="ShowCoverImage(this.parentNode,false);"
                                onmouseout="HidePrevImage();"
                                onclick="ShowCoverImage(this.parentNode,true);">&#x4e;
                        </button>
                        <label unselectable="on">
                            <xsl:value-of select="$IDS_IMAGE"/>
                        </label>
            ...
            <br/>
            <label unselectable="on" class="hl">
                <xsl:value-of select="$IDS_LANGUAGE"/>
            </label>
            <!--input type="text" maxlength="2" id="tiLang" class="short" value="{$cur/f:lang}"/-->

            <select id="tiLang" title="Language of the text"
                    onchange="document.getElementById('fbw_updater').innerHTML=document.getElementById('fbw_updater').innerHTML=='0'?'1':'0';">
                <xsl:call-template name="fill_languages_combo">
                    <xsl:with-param name="lang" select="$cur/f:lang"/>
                </xsl:call-template>
            </select>

            <label unselectable="on" style="width: 10em">
                <xsl:value-of select="$IDS_SOURCE_LANG"/>
            </label>
            ...
            <fieldset id="tiTrans" unselectable="on" class="kid">
                <legend unselectable="on">
                    <xsl:value-of select="$IDS_TRANSLATORS"/>
                </legend>
            ...
            <fieldset id="tiSeq" unselectable="on" class="kid">
                <br/>
                <legend unselectable="on">
                    <xsl:value-of select="$IDS_SEQUENCE"/>
                </legend>
    ...
    <xsl:template name="sti">
        ...
        <fieldset unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_STI"/>
                <xsl:if test="not($cur)">
                    <button onclick="ShowElement(this.parentNode.parentNode.parentNode.id, false)" unselectable="on"
                            style="font-family : Tahoma; float:none">-
                    </button>
                </xsl:if>
            </legend>
            <fieldset id="stiGenre" unselectable="on" class="kid">
                ...
                <xsl:if test="not($cur/f:genre)">
                    <div unselectable="on">
                        ...
                        <input type="text" maxlength="3" id="match" value="" class="short"/>
                    </div>
                </xsl:if>
            </fieldset>
            ...
            <input type="text" maxlength="256" id="stiTitle" class="wide" value="{$cur/f:book-title}"/>
            <br/>
            <label unselectable="on">
                <xsl:value-of select="$IDS_KEYWORDS"/>
            </label>
            <input type="text" maxlength="256" id="stiKwd" class="wide" value="{$cur/f:keywords}"/>
            <br/>
            <label unselectable="on">
                <xsl:value-of select="$IDS_DATE_TEXT"/>
            </label>
            ...
            <input type="text" maxlength="256" id="stiDateVal" value="{$cur/f:date/@value}"/>
            <br/>
    ...
    <xsl:template name="di">
        ...
        <fieldset unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_DOCUMENT"/>
            </legend>
            <fieldset id="diAuthor" unselectable="on" class="kid">
                ...
                <span id="di_id" unselectable="on">
                    <button style="font-family:Tahoma,Arial;margin-right:0.6em;" unselectable="on">
                        <xsl:attribute name="onclick">NewDocumentID("<xsl:value-of select="$IDS_ID_WARN"></xsl:value-of>")
                        </xsl:attribute>
            ...
            <br/>
            <label unselectable="on">
                <xsl:value-of select="$IDS_TOOLS_USED"/>
            </label>
            ...
            <br/>
            <label unselectable="on" style="width:6.7em">
                <xsl:value-of select="$IDS_SOURCE_OCR"/>
            </label>
            <input type="text" maxlength="256" id="diOCR" class="wide" value="{$cur/f:src-ocr}"/>
            <br/>
            <fieldset id="diURL" unselectable="on" class="kid">
                <legend unselectable="on">
                    <xsl:value-of select="$IDS_SOURCE_URL"/>
                </legend>
    ...
    <xsl:template name="pi">
        ...
        <fieldset unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_PAPER_BOOK"/>
            </legend>
            <label unselectable="on">
                <xsl:value-of select="$IDS_BOOK_TITLE"/>
            </label>
            <input type="text" maxlength="256" id="piName" class="wide" value="{$cur/f:book-name}"/>
            <br/>
            <label unselectable="on">
                <xsl:value-of select="$IDS_PUBLISHER"/>
            </label>
            <input type="text" maxlength="256" id="piPub" class="wide" value="{$cur/f:publisher}"/>
            <br/>
            <br/>
            <label unselectable="on">
                <xsl:value-of select="$IDS_CITY"/>
            </label>
            <input type="text" maxlength="256" id="piCity" value="{$cur/f:city}"/>
            <label unselectable="on" class="short">
                <xsl:value-of select="$IDS_YEAR"/>
            </label>
            <input type="text" maxlength="256" id="piYear" style="width:6em" value="{$cur/f:year}"/>
            <br/>
            <label unselectable="on" class="short">
                <xsl:value-of select="$IDS_ISBN"/>
            </label>
            <input type="text" maxlength="256" id="piISBN" class="wide" value="{$cur/f:isbn}"/>
            <br/>
            <br/>
    ...
    <xsl:template name="custom-info">
        ...
        <fieldset id="ci" unselectable="on">
            <legend unselectable="on" class="top">
                <xsl:value-of select="$IDS_CUSTOM_INFO"/>
            ...
            <xsl:for-each select="$items">
                <div unselectable="on">
                    ...
                    <button onclick="Clone(this.parentNode)" unselectable="on">&#x32;</button>
                    <label unselectable="on">
                        <xsl:value-of select="$IDS_TYPE"/>
                    </label>
                    <input type="text" maxlength="256" id="type" value="{@info-type}"/>
                    <br/>
                    <span class="top" unselectable="on">
                        <label unselectable="on">
                            <xsl:value-of select="$IDS_VALUE"/>
                        </label>
                    </span>
            ...
            <xsl:if test="not($items)">
                <div unselectable="on">
                    ...
                    <input type="text" maxlength="256" id="type" value=""/>
                    <br/>
                    <span class="top" unselectable="on">
                        <label unselectable="on">
                            <xsl:value-of select="$IDS_VALUE"/>
