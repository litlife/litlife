...
@section('content')
	...
	<div class="card">
		<div class="card-body">
			<form role="form" method="POST" action="{{ route('author_repeats.update', $authorRepeat) }}"
				...
				<div class="row form-group{{ $errors->has('authors') ? ' has-error' : '' }}">
					...
					<div class="col-md-9 col-lg-10">
						<select id="authors" name="authors[]" class="authors form-control select2-multiple" multiple
							...
							@if($authorRepeat->authors->count())
								@foreach($authorRepeat->authors as $c => $author)
				...
				<div class="row form-group{{ $errors->has('comment') ? ' has-error' : '' }}">
					...
					<div class="col-md-9 col-lg-10">
                        <textarea id="comment" class="form-control" rows="5"
								  name="comment">{!! $authorRepeat->comment ?? old('comment') !!}</textarea>
					</div>
				</div>
