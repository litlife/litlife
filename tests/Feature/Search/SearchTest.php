...
class SearchTest extends TestCase
{
	...
	public function testResultMoreUrls()
	{
		...
		$this->get(route('search', ['query' => $query]))
			->assertOk()
			->assertViewHas('books_url', route('books', ['search' => $query, 'order' => 'rating_avg_down',
	...
	public function testMinLength()
	{
		...
		$this->get(route('search', ['query' => $query]))
			->assertOk()
			->assertSeeText(trans_choice('search.minimum_length_of_the_search_string', config('litlife.minimum_number_of_letters_and_numbers'), ['characters_count' => config('litlife.minimum_number_of_letters_and_numbers')]));
	}
