...
class BookSaleTest extends TestCase
{
    ...
    public function testChangePriceCooldown()
    {
        ...
        $response->assertSessionHasErrors([
            'price' =>
                trans_choice('book.book_price_cant_changed_within_period_days', config('litlife.book_price_update_cooldown'),
                    ['days_count' => config('litlife.book_price_update_cooldown')])
        ]);
    ...
    public function testMinimumCharactersCountBeforeBookCanBeSold()
    {
        ...
        $this->actingAs($user)
            ...
            ->assertSessionHasErrors([
                'error' => __('book.minimum_characters_count_before_book_can_be_sold',
    ...
    public function testForSaleMustHaveAnnotation()
    {
        ...
        $response = $this->actingAs($user)
            ->get(route('books.sales.edit', $book))
            ->assertOk()
            ->assertSeeText(__('book.annotation_must_contain_at_least_characters_for_sale',
        ...
        $this->assertSessionHasErrors(__('book.annotation_must_contain_at_least_characters_for_sale',
            ['characters_count' => config('litlife.min_annotation_characters_count_for_sale')]));
        ...
        $response = $this->actingAs($user)
            ->get(route('books.sales.edit', $book))
            ->assertOk()
            ->assertDontSeeText(__('book.annotation_must_contain_at_least_characters_for_sale',
                ['characters_count' => config('litlife.min_annotation_characters_count_for_sale')]));
    }
    ...
    public function testBookWillNotBeSoldIfTheNumberOfFreeChaptersIsGreaterThanOrEqualToTheNumberOfChapters()
    {
        ...
        $this->actingAs($user)
            ->get(route('books.sales.edit', ['book' => $book]))
            ->assertOk()
            ->assertDontSeeText(__('book.the_book_will_not_be_sold_if_the_number_of_free_chapters_is_greater_than_or_equal_to_the_number_of_chapters',
        ...
        $this->actingAs($user)
            ->get(route('books.sales.edit', ['book' => $book]))
            ->assertOk()
            ->assertSeeText(__('book.the_book_will_not_be_sold_if_the_number_of_free_chapters_is_greater_than_or_equal_to_the_number_of_chapters',
                ['sections_count' => $book->sections_count]));
        ...
        $this->actingAs($user)
            ...
            ->assertRedirect()
            ->assertSessionHasErrors([
                'free_sections_count' => __('book.the_book_will_not_be_sold_if_the_number_of_free_chapters_is_greater_than_or_equal_to_the_number_of_chapters',
                    ['sections_count' => $book->sections_count])
            ]);
    ...
    public function testTheBookShouldNotHaveChaptersWithLongerCharactersThanAllowedSoThatItCanBeSold()
    {
        ...
        $this->assertSessionHasErrors(__('book.book_should_be_divided_into_chapters_and_parts',
            ['max_symbols_count' => config('litlife.max_section_characters_count')]));
        $this->assertEquals(0, $book->price);
    }
