...
class TopicTest extends DuskTestCase
{
    ...
    public function testSeeLabelsIfIdeaForum()
    {
        $this->browse(function ($user_browser) {
            ...
            $topic = Topic::factory()->idea_implemented()->create(['forum_id' => $forum->id]);

            $topic2 = Topic::factory()->idea_on_review()->create(['forum_id' => $forum->id]);

            $topic3 = Topic::factory()->idea_rejected()->create(['forum_id' => $forum->id]);

            $topic4 = Topic::factory()->idea_in_progress()->create(['forum_id' => $forum->id]);
