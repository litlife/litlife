...
class AnnotationCkeditorContentFilterTest extends DuskTestCase
{
	/**
	 * A Dusk test example.
	 *
	...
	public function testH()
	{
		...
		$this->filterCkeditor('<div class="u-section-break">text</div>', '<div>text</div>');
		...
		if (empty($this->browser)) {
			$this->browse(function ($browser) {
				...
				$browser->resize(1000, 1000)
					->loginAs($admin)
					->visit(route('books.edit', ['book' => $book]))
	...
	public function filterCkeditor($input, $output = null)
	{
		...
		$this->browser->driver->executeScript('CKEDITOR.instances.annotation.setData( "' . addslashes($input) . '" );');
		$this->browser->pause(20);
		$this->assertEquals($output, trim($this->browser->driver->executeScript('return CKEDITOR.instances.annotation.getData();')));
	}
}
